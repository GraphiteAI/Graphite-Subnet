...
static void Read_GCTSP_SECTION()
{
    ...
    assert(ColorAllowed = (short **) malloc((Salesmen + 1) * sizeof(short *)));
    for (j = 1; j <= Salesmen; j++) {
        assert(ColorAllowed[j] = (short *)
               malloc((Dimension + 1) * sizeof(short)));
...
static void Read_GCTSP_SET_SECTION()
{
    ...
    assert(Used = (char *) calloc(Dimension + 1, sizeof(char)));
    assert(ColorAllowed = (short **) malloc((Salesmen + 1) * sizeof(short *)));
    for (j = 1; j <= Salesmen; j++)
        assert(ColorAllowed[j] = (short *)
                                 malloc((Dimension + 1) * sizeof(short)));
