...
import unittest
from graphite.validator.reward_copy import scaled_rewards
...
class TestRewards(unittest.TestCase):
    ...
    def test_unsolved_all_inf(self):
        ...
        self.assertEquals(mock_rewards, [0,0,0])
        
    def test_unsolved_all_better(self):
        mock_scores = [1, 6, 11]
        mock_benchmark = np.inf
        mock_rewards = scaled_rewards(mock_scores, mock_benchmark, 'min')
        [self.assertAlmostEquals(val1,val2) for val1, val2 in zip(mock_rewards,[1,0.6,0.2])]
    
    def test_unsolved_all_better_negative(self):
        mock_scores = [-1, -6, -11]
        mock_benchmark = np.inf
        mock_rewards = scaled_rewards(mock_scores, mock_benchmark, 'min')
        [self.assertAlmostEquals(val1,val2) for val1, val2 in zip(mock_rewards,[0.2,0.6,1])]
    ...
    def test_unsolved_some_better(self):
        ...
        mock_benchmark = np.inf
        mock_rewards = scaled_rewards(mock_scores, mock_benchmark, 'min')
        [self.assertAlmostEquals(val1,val2) for val1, val2 in zip(mock_rewards,[1,0.2,0,1])]
    ...
    def test_solved_some_better(self):
        ...
        mock_benchmark = 11
        mock_rewards = scaled_rewards(mock_scores, mock_benchmark, 'min')
        [self.assertAlmostEquals(val1,val2) for val1, val2 in zip(mock_rewards,[1,0.6,0])]

    def test_solved_all_worse(self):
        mock_scores = [1, 6, 12]
        mock_benchmark = 0
        mock_rewards = scaled_rewards(mock_scores, mock_benchmark, 'min')
        [self.assertAlmostEquals(val1,val2) for val1, val2 in zip(mock_rewards,[0,0,0])] 
